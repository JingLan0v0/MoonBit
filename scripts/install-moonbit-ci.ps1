# CI-only isolated install. Pin the compiler and retry official download mirrors.
...
$installRoot = Join-Path $env:RUNNER_TEMP 'moonrow-toolchain'
$downloadRoot = Join-Path $env:RUNNER_TEMP 'moonrow-downloads'
if (Test-Path -LiteralPath $installRoot) { throw 'Toolchain destination must be fresh.' }
...
function Get-OfficialFile([string]$RelativePath, [string]$Destination) {
  foreach ($origin in @('https://cli.moonbitlang.com', 'https://cli.moonbitlang.cn')) {
    ...
    Write-Host "Retrying official mirror for $RelativePath"
  }
  throw "Could not download $RelativePath from either official mirror."
}
...
foreach ($line in Get-Content -LiteralPath $checksums) {
  ...
  if (-not $target.StartsWith($binRoot + [IO.Path]::DirectorySeparatorChar, [StringComparison]::OrdinalIgnoreCase)) {
    throw 'Checksum entry escapes binary directory.'
  }
  if ((Get-FileHash -LiteralPath $target -Algorithm SHA256).Hash.ToLower() -ne $expected) {
...
try {
  & (Join-Path $binRoot 'moon.exe') bundle --target js --warn-list '-a'
...
Write-Host "Pinned MoonBit installed; $verified binary checksums verified."
